<template>
  <div class="d-inline-flex">
    <v-btn
      :color="selectedGenres?.length > 0 ? '' : ''"
      :variant="selectedGenres?.length > 0 ? 'flat' : 'outlined'"
      rounded="pill"
      class="text-caption px-6"
      @click="openSelector"
      >Genero
      <span class="ml-1" v-if="selectedGenres?.length > 0"
        >({{ selectedGenres.length }})</span
      ></v-btn
    >
    <v-navigation-drawer
      class="navigation text-left"
      floating
      temporary
      width="350"
      location="right"
    ...
    >
      ...
      <template v-slot:prepend>
        ...
        <v-text-field
          variant="plain"
          class="pl-4 font-weight-bold text-h1"
          placeholder="Procure por um genero"
          v-model="search"
        ></v-text-field>
      </template>
      ...
      <v-item-group multiple v-model="selectedGenresLocal">
        ...
        >
          <v-list-item
            :disabled="eventsStore.getCountByGenre(genre) === 0 ? true : false"
            class="py-1"
      ...
      <template v-slot:append>
        ...
        >
          <v-btn
            ...
            rounded="pill"
            @click="restabelecer"
            :disabled="!selectedGenresLocal.length"
            >Resetar</v-btn
          >
...
async function buscar() {
  await router.push({ query: { genre: selectedGenresLocal.value } });
