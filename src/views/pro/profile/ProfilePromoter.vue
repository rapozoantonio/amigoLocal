<template>
    ...
    <form-card v-if="user" @submit="saveProfile" :schema="promoterSchema" v-model:model="user" v-model:files="files"
        labelType="up" title="Edit Promoter Info" action="Save" :items="{ language: languages, gender: genders }">
...
async function saveProfile(event) {
    try {
        ...
        const response = await firebaseStore.putDocument("promoters", user.value.uid, user.value);
